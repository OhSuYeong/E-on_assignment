...
def fast() : #빠른 시간 기차 검색 및 예매
    ...
    for i in range(0,len(data)) : #리스트에서 리스트 길이만큼 반복해서
        if (new[1] == data[i][1]) and (new[2] == data[i][3]) and (new[3] == data[i][4]) and (data[i][5] != '매진') :    #출발역과 도착역과 열차종류가 같고 매진이 아닌 경우
...
def alllist() : #전체 기차 리스트 출력
    for i in range(0,len(data)) : #리스트 길이만큼 반복
        print("시간 :", data[i][0], data[i][1], data[i][2], data[i][3], "/ 열차종류 :", data[i][4], "/ 잔여좌석수 :", data[i][5])
